...
export const DEFAULT_GOOGLE_CLIENT_ID =
  ...
  '185693206909-vitr06gaei3cc1acoqo59h7hkt4d4t0n.apps.googleusercontent.com';

export const getGoogleClientId = (): string => {
  return localStorage.getItem('catatan_guru_google_client_id') || DEFAULT_GOOGLE_CLIENT_ID;
};

export const setGoogleClientId = (clientId: string) => {
  if (!clientId || !clientId.trim()) {
    localStorage.removeItem('catatan_guru_google_client_id');
  } else {
    localStorage.setItem('catatan_guru_google_client_id', clientId.trim());
  }
};
...
export const initAuth = (
  ...
  // 1. Cek sesi Google Identity Services dari localStorage
  if (typeof window !== 'undefined') {
    const storedUser = localStorage.getItem('catatan_guru_google_user');
    const storedToken = localStorage.getItem('catatan_guru_google_token');
    if (storedUser && storedToken) {
      try {
        const parsed = JSON.parse(storedUser);
        cachedAccessToken = storedToken;
        if (onAuthSuccess) onAuthSuccess(parsed, storedToken);
        return () => {};
      } catch {
  ...
  // 2. Fallback ke Firebase Auth state
...
export const googleSignIn = async (): Promise<{ user: GoogleAuthUser; accessToken: string } | null> => {
  ...
  try {
    // 1. Prioritaskan Google Identity Services (Google Cloud Console OAuth murni tanpa batasan Firebase)
    await loadGsiScript();
    ...
    if (google?.accounts?.oauth2 && clientId) {
      try {
        const gsiResult = await new Promise<{ user: GoogleAuthUser; accessToken: string }>((resolve, reject) => {
          let isHandled = false;
          try {
              ...
              callback: async (response: any) => {
                ...
                if (response.error) {
                  if (response.error === 'access_denied') {
                    reject(new Error('Akses Google ditolak atau proses login dibatalkan.'));
                  } else if (response.error === 'idpiframe_initialization_failed') {
                    reject(
                      new Error(
                        `Domain "${window.location.origin}" belum diizinkan di Authorized JavaScript origins pada Google Cloud Console Client ID.`
                      )
                    );
                  } else {
                    reject(
                      new Error(
                        `Google Sign-In: ${response.error_description || response.error}`
                      )
                    );
                  }
                  return;
                ...
                if (!response.access_token) {
                  reject(new Error('Tidak menerima Access Token dari Google Sign-In.'));
                  return;
                }
                ...
                try {
                  ...
                  console.warn('Gagal memuat profil pengguna Google:', uErr);
                }

                if (typeof window !== 'undefined') {
                  localStorage.setItem('catatan_guru_google_user', JSON.stringify(userInfo));
                  localStorage.setItem('catatan_guru_google_token', accessToken);
                }
              ...
              error_callback: (err: any) => {
                if (!isHandled) {
                  ...
                  reject(
                    new Error(
                      err?.message ||
                        'Gagal membuka otorisasi Google. Pastikan popup tidak diblokir.'
                    )
                  );
                }
              },
        ...
        console.warn('GSI Auth error, checking details:', gsiErr);
        throw gsiErr;
      }
    }

    // 2. Fallback Firebase Auth jika GSI belum siap
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('catatan_guru_google_user', JSON.stringify(gUser));
    ...
    if (error?.code === 'auth/unauthorized-domain' || String(error?.message).includes('unauthorized-domain')) {
      const currentHost = typeof window !== 'undefined' ? window.location.hostname : 'domain webapp Anda';
      throw new Error(`Domain "${currentHost}" belum diizinkan di Firebase Console. Beralihlah ke Google Cloud Console atau gunakan mode Google Apps Script.`);
    }
...
export const logoutGoogle = async () => {
  cachedAccessToken = null;
  if (typeof window !== 'undefined') {
    localStorage.removeItem('catatan_guru_google_user');
    localStorage.removeItem('catatan_guru_google_token');
  }
  try {
    await signOut(auth);
  } catch {
    // Abaikan jika tidak ada sesi firebase aktif
  }
};
