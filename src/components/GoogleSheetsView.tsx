...
export const GoogleSheetsView: React.FC<GoogleSheetsViewProps> = ({
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (msg.includes('Authorized Domains') || msg.includes('belum diizinkan') || msg.includes('unauthorized-domain')) {
        setUnauthorizedDomain(window.location.hostname);
      }
      showToast(`Gagal masuk Google: ${msg}`, 'error');
    } finally {
    ...
    <div className="space-y-6 pb-12">
      ...
      {activeIntegrationMode === 'oauth' && (
        <div className="space-y-6">
          ...
          {/* Alert Panduan Otorisasi Google Cloud Console */}
          {unauthorizedDomain && !user && (
            <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800/80 rounded-2xl p-5 space-y-4">
              <div className="flex items-start space-x-3">
                <AlertCircle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <h4 className="text-sm font-bold text-amber-900 dark:text-amber-200">
                    Otorisasi Google Cloud Console
                  </h4>
                  <p className="text-xs text-amber-800 dark:text-amber-300/90 leading-relaxed">
                    Agar login Google dapat berjalan langsung tanpa Firebase, URL origin aplikasi ini perlu didaftarkan di <strong>Authorized JavaScript origins</strong> pada Google Cloud Console Anda.
                  </p>
                </div>
              </div>

              <div className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-900/50 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block mb-0.5">
                    URL Origin yang Harus Ditambahkan:
                  </span>
                ...
                <button
                  onClick={() => {
                    ...
                    showToast('URL Origin berhasil disalin!', 'success');
                    setTimeout(() => setCopiedDomain(false), 2500);
                  }}
                  className="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/50 hover:bg-amber-200 dark:hover:bg-amber-800 text-amber-900 dark:text-amber-200 text-xs font-semibold rounded-lg flex items-center justify-center space-x-1.5 transition-colors shrink-0"
                >
                  {copiedDomain ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  <span>{copiedDomain ? 'Tersalin!' : 'Salin Origin'}</span>
                </button>
              </div>

              <div className="text-xs text-amber-900 dark:text-amber-200 space-y-2 pl-1">
                <p className="font-semibold">Langkah Pengaturan di Google Cloud Console:</p>
                <ol className="list-decimal list-inside space-y-1.5 text-amber-800 dark:text-amber-300/90">
                  <li>
                    Buka{' '}
                    <a
                      ...
                      className="inline-flex items-center space-x-1 text-indigo-600 dark:text-indigo-400 font-bold underline hover:text-indigo-700"
                    >
                      <span>Google Cloud Console Credentials</span>
                      <ExternalLink className="w-3 h-3" />
                    </a>
                    , pastikan project Anda (<strong>teacheers</strong>) terpilih di bagian atas.
                  </li>
                  <li>
                    Klik nama <strong>OAuth 2.0 Client ID</strong> Anda. Pada bagian <strong>Authorized JavaScript origins</strong>, klik <strong>Add URI</strong> dan tempelkan: <code className="bg-amber-100 dark:bg-amber-900/60 px-1 py-0.5 rounded font-mono">{window.location.origin}</code>, lalu klik <strong>Save</strong>.
                  </li>
                  <li>
                    <strong>Agar semua pengguna bisa login tanpa mendaftarkan Test User:</strong> Buka menu <strong>OAuth consent screen</strong> di sebelah kiri, lalu klik tombol <strong>Publish App</strong> (Publikasikan aplikasi) agar statusnya berubah dari <em>Testing</em> menjadi <em>In production</em>.
                  </li>
                  <li>
                    <em>Solusi Paling Cepat & Bebas Ribet:</em> Gunakan tab <strong>Google Apps Script (GAS)</strong> di bagian atas. Mode GAS 100% tidak memerlukan Client ID, pengaturan domain, ataupun pendaftaran user test sama sekali!
                  </li>
                </ol>
              </div>
            </div>
