...
export const AppearanceLayout: React.FC<AppearanceLayoutProps> = ({
  ...
  useEffect(() => {
    ...
    localStorage.setItem('app_visual_style', visualStyle);
    ...
    window.dispatchEvent(
      new CustomEvent('app_appearance_changed', {
        detail: { fontSize, glassBlur, glassOpacity, bgImage, bgStyle, bgOpacity, visualStyle },
      })
    );
  }, [fontSize, glassBlur, glassOpacity, bgOpacity, bgStyle, bgImage, visualStyle]);

  // Handle Visual Style Change
  const handleSelectVisualStyle = (style: VisualStyle) => {
    setVisualStyle(style);
    localStorage.setItem('app_visual_style', style);
    document.documentElement.setAttribute('data-visual-style', style);
    window.dispatchEvent(
      new CustomEvent('app_appearance_changed', {
        detail: { fontSize, glassBlur, glassOpacity, bgImage, bgStyle, bgOpacity, visualStyle: style },
      })
    );
    if (onShowToast) {
      onShowToast(
        style === 'solid'
          ? 'Gaya visual White Solid berhasil diterapkan!'
          : 'Gaya visual Glassmorphism berhasil diterapkan!',
        'success'
      );
    }
  };
  ...
  const handleSelectMode = (isDark: boolean) => {
    ...
    if (onShowToast) {
      onShowToast(
        isDark ? 'Mode Gelap (Dark Mode) aktif!' : 'Mode Terang (Light Mode) aktif!',
        'success'
      );
    }
  };
  ...
  const cardBaseClass = isSolid
    ? 'rounded-2xl p-6 sm:p-7 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 shadow-sm transition-all'
    : 'rounded-3xl p-6 sm:p-7 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/70 dark:border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.06)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] transition-all';

  const subCardClass = isSolid
    ? 'bg-[#F9FAFC] dark:bg-slate-800/50 p-4 sm:p-5 rounded-xl border border-gray-200 dark:border-slate-800'
    : 'bg-white/50 dark:bg-white/5 p-4 sm:p-5 rounded-2xl border border-white/60 dark:border-white/10';
  ...
  return (
    <div className="space-y-8 pb-20 max-w-6xl mx-auto">
      ...
      <div className={`relative overflow-hidden ${
        isSolid
          ? 'rounded-2xl p-6 sm:p-8 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 shadow-sm'
          : 'rounded-3xl p-6 sm:p-8 bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/70 dark:border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.06)] dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]'
      }`}>
        {!isSolid && (
          <>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 font-medium max-w-2xl">
              Pilih gaya visual <strong>White Solid</strong> atau <strong>Glassmorphism</strong>, atur tema warna, ukuran font keseluruhan, serta pasang foto latar belakang kustom.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 self-start md:self-auto">
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-slate-700 dark:text-white text-xs font-bold transition-all shadow-sm active:scale-95 ${
                isSolid
                  ? 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-gray-200 dark:border-slate-700'
                  : 'bg-white/60 hover:bg-white dark:bg-white/10 dark:hover:bg-white/20 border border-slate-200 dark:border-white/20'
              }`}
            >
      ...
      {/* 1. VISUAL STYLE SELECTOR: White Solid vs Glassmorphism */}
      <div className={`${cardBaseClass} space-y-4`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-gray-200 dark:border-slate-800">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-indigo-500/10 dark:bg-indigo-500/20 border border-indigo-500/20 dark:border-indigo-400/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
              <Palette className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                Pilihan Visual Style
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Pilih konsep arsitektur visual: Solid minimalis atau Glassmorphism
              </p>
            </div>
          </div>
          <span className="self-start sm:self-auto px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20">
            Aktif: {visualStyle === 'solid' ? 'White Solid' : 'Glassmorphism'}
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
          {/* Option 1: White Solid */}
          <button
            type="button"
            onClick={() => handleSelectVisualStyle('solid')}
            className={`relative p-5 rounded-2xl border transition-all text-left flex flex-col justify-between group ${
              visualStyle === 'solid'
                ? 'bg-white dark:bg-slate-900 border-indigo-600 dark:border-indigo-500 ring-2 ring-indigo-500/40 shadow-lg shadow-indigo-500/10'
                : isSolid
                  ? 'bg-[#F9FAFC] hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/80 border-gray-200 dark:border-slate-800'
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2.5">
                  <div className="w-9 h-9 rounded-xl bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-white border border-gray-300 dark:border-slate-700 flex items-center justify-center shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Solid Clean & Minimalist</p>
                  </div>
                </div>
                {visualStyle === 'solid' && (
                  <span className="flex items-center space-x-1 text-xs font-bold text-indigo-600 bg-indigo-50 dark:bg-indigo-950/60 dark:text-indigo-400 px-2.5 py-0.5 rounded-full border border-indigo-200 dark:border-indigo-800">
              ...
              <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
                <p className="leading-relaxed">
                  Struktur visual tegas dengan panel putih bersih, garis tepi tegas, dan latar solid (#F9FAFC) tanpa efek blur transparan.
                </p>
              </div>
            </div>
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-bold text-indigo-600 dark:text-indigo-400">
              <span>Klik untuk terapkan White Solid</span>
          ...
          <button
            type="button"
            onClick={() => handleSelectVisualStyle('glass')}
            className={`relative p-5 rounded-2xl border transition-all text-left flex flex-col justify-between group ${
              visualStyle === 'glass'
                ? 'bg-white/80 dark:bg-slate-900/90 border-blue-500 dark:border-blue-400 ring-2 ring-blue-500/40 shadow-lg shadow-blue-500/20'
                : isSolid
                  ? 'bg-[#F9FAFC] hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/80 border-gray-200 dark:border-slate-800'
                  : 'bg-white/30 hover:bg-white/50 border-white/60 dark:bg-slate-800/30 dark:hover:bg-slate-800/60 dark:border-white/10'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2.5">
                  <div className="w-9 h-9 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 flex items-center justify-center shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Frosted Glass & Ambient Orbs</p>
                  </div>
                </div>
                {visualStyle === 'glass' && (
                  <span className="flex items-center space-x-1 text-xs font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/60 dark:text-blue-400 px-2.5 py-0.5 rounded-full border border-blue-200 dark:border-blue-800">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Dipilih</span>
                  </span>
                )}
              ...
              <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
                <p className="leading-relaxed">
                  Tampilan kaca transparan modern (*frosted crystal*), efek blur halus, pendaran ambient orbs, dan kedalaman bayangan 3D.
                </p>
              </div>
            </div>
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-bold text-blue-600 dark:text-blue-400">
              <span>Klik untuk terapkan Glassmorphism</span>
              <Sparkle className="w-3.5 h-3.5" />
            </div>
          </button>
        </div>
      ...
      {/* 2. THEME MODE SELECTOR: Light Mode vs Dark Mode */}
      <div className={`${cardBaseClass} space-y-4`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-gray-200 dark:border-slate-800">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                Pilihan Mode Tema (Warna)
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Pilih nuansa terang (Light) atau gelap (Dark) sesuai kenyamanan mata Anda
              </p>
            </div>
          </div>
          <span className="self-start sm:self-auto px-3 py-1 rounded-full text-xs font-bold bg-violet-500/10 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300 border border-violet-500/20">
            Aktif: {darkMode ? 'Dark Mode' : 'Light Mode'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
          ...
          <button
            ...
            className={`relative p-5 rounded-2xl border transition-all text-left flex flex-col justify-between group ${
              !darkMode
                ? 'bg-white dark:bg-slate-900 border-amber-500 ring-2 ring-amber-500/40 shadow-lg shadow-amber-500/10'
                : isSolid
                  ? 'bg-[#F9FAFC] hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/80 border-gray-200 dark:border-slate-800'
                  : 'bg-white/30 hover:bg-white/50 border-white/60 dark:bg-slate-800/30 dark:hover:bg-slate-800/60 dark:border-white/10'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Mode Terang</p>
                  </div>
                </div>
                {!darkMode && (
                  <span className="flex items-center space-x-1 text-xs font-bold text-amber-700 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Dipilih</span>
                  </span>
                )}
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Tampilan bernuansa putih/terang dengan tipografi hitam berdaya baca tinggi untuk kenyamanan kerja siang hari.
              </p>
            </div>
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-bold text-amber-600 dark:text-amber-400">
              <span>Klik untuk terapkan Light Mode</span>
              <Sparkle className="w-3.5 h-3.5" />
            </div>
          </button>
          ...
          <button
            ...
            className={`relative p-5 rounded-2xl border transition-all text-left flex flex-col justify-between group ${
              darkMode
                ? 'bg-slate-900 border-violet-500 ring-2 ring-violet-500/40 shadow-lg shadow-violet-500/20'
                : isSolid
                  ? 'bg-[#F9FAFC] hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800/80 border-gray-200 dark:border-slate-800'
                  : 'bg-white/30 hover:bg-white/50 border-white/60 dark:bg-slate-800/30 dark:hover:bg-slate-800/60 dark:border-white/10'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Mode Gelap</p>
                  </div>
                </div>
                {darkMode && (
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Tampilan gelap elegan (*deep slate*), ramah di mata, mengurangi silau layar saat malam hari.
              </p>
            </div>
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-bold text-violet-600 dark:text-violet-400">
              <span>Klik untuk terapkan Dark Mode</span>
              <Sparkle className="w-3.5 h-3.5" />
            </div>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className={`flex flex-col justify-between ${cardBaseClass} space-y-6`}>
          <div className="space-y-6">
            ...
            {/* Section 1: Ukuran Font Keseluruhan (12px - 20px) */}
            <div className={`space-y-3 ${subCardClass}`}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-white border border-gray-200 dark:border-slate-700 font-mono text-xs font-bold shadow-sm">
                    {fontSize}px
                  </span>
                </div>
              </div>
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                      fontSize === sz
                        ...
                        : isSolid
                          ? 'bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-white border border-gray-200 dark:border-slate-700'
            ...
            <div className={`space-y-4 ${subCardClass}`}>
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-500 dark:text-slate-400 font-mono font-medium">
                  {isSolid ? 'Mode Solid Aktif' : 'Blur & Transparansi'}
                </span>
              </div>
              ...
              {/* Slider 2: Opacity */}
              <div className="space-y-2 pt-2 border-t border-gray-200 dark:border-slate-750">
                <div className="flex justify-between text-xs font-medium">
            ...
            <div
              className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${
                isSolid ? 'bg-[#F9FAFC] dark:bg-slate-800/80 border-gray-200 dark:border-slate-700 shadow-sm' : ''
              }`}
              style={
                !isSolid
                  ? {
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-white">
                    {isSolid ? 'Preview White Solid' : `Live Glass Preview (${darkMode ? 'Dark Glass' : 'Light Glass'})`}
                  </span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-300 font-medium">
                  {isSolid
                    ? 'Mode White Solid: latar solid, kartu berkontras tajam, panel tanpa efek blur.'
          ...
          <div className="space-y-6">
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-2xl p-5 text-center cursor-pointer transition-all flex flex-col items-center justify-center space-y-2 ${
                  isDragging
                    ...
                    : isSolid
                      ? 'border-gray-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-orange-400'
              ...
              >
                ...
                {bgImage ? (
                  <div className="w-full space-y-3">
                    <div className="relative h-28 w-full rounded-xl overflow-hidden border border-gray-200 dark:border-slate-700 shadow-inner group">
                      <img
                        src={bgImage}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className={`w-full px-4 py-2.5 rounded-xl text-slate-800 dark:text-white text-xs sm:text-sm font-semibold focus:outline-none focus:border-orange-500 cursor-pointer shadow-sm ${
                  isSolid
                    ? 'bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700'
                    : 'bg-white/90 dark:bg-slate-800/90 border border-slate-300 dark:border-white/20'
                }`}
              >
            ...
            <div className={`space-y-2 ${subCardClass}`}>
              <div className="flex justify-between text-xs font-medium">
